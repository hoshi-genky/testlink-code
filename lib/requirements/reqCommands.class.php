...
class reqCommands {
  ...
  */
  function doCreate(&$argsObj,$request)
  {
    ...
    if($ret['status_ok'])
    {
      ...
      {
        $obj->template = "reqView.php?refreshTree={$argsObj->refreshTree}&tproject_id={$argsObj->tproject_id}&requirement_id={$ret['id']}";
      }
      $obj->req_id = $ret['id'];
      $argsObj->scope = '';
    }
  ...
  function doUpdate(&$argsObj,$request)
  {
    ...
    else if( $diff['nochange'] || ( ($createRev = $diff['force'] && !$obj->prompt_for_log) || $argsObj->do_save ) )
    {
      ...
      if($ret['status_ok'])
      {
        $obj->main_descr = '';
        $obj->action_descr = '';
        $obj->template = "reqView.php?refreshTree={$argsObj->refreshTree}&tproject_id={$argsObj->tproject_id}&requirement_id={$argsObj->req_id}";
  ...
  */
  ...
  public function doAddRelation($argsObj,$request) 
  {
    ...
    $obj->template = "reqView.php?requirement_id={$own_id}&tproject_id={$argsObj->tproject_id}&relation_add_result_msg=" . $op['msg'];
  ...
  public function doDeleteRelation($argsObj,$request) 
  {
    ...
    $obj = $this->initGuiBean();    
    $obj->template = "reqView.php?requirement_id=$requirement_id&tproject_id={$argsObj->tproject_id}&relation_add_result_msg=" . $op['msg'];
  ...
  */
  ...
  function addTestCase(&$argsObj,$request) {
    ...
    $obj->req_spec_id = $argsObj->req_spec_id;
    $obj->req_version_id = $argsObj->req_version_id;
    $obj->template = "reqView.php?refreshTree=0&tproject_id={$argsObj->tproject_id}&requirement_id={$argsObj->req_id}";
  ...
  private function initGuiObjForAttachmentOperations($argsObj) {
    ...
    $guiObj->template = "reqView.php?refreshTree=0&tproject_id={$argsObj->tproject_id}&requirement_id={$argsObj->req_id}";
