...
<p><i>テスト仕様</i> は、存在する<i>テスト集合</i>そして<i>テストケース</i> の".
        "情報を閲覧したり変更したりする場所です。" .
        "異なるバージョンのテストケースを見ることもできます。</p>

<h2>やってみましょう！:</h2>
<ol>
  <li>プロジェクトをナビゲーションのツリーから選択します。<i>メモ: " .
  "プロジェクトの変更は右上の" .
    "プルダウンリストを選択することでおこなうことができます。</i></li>
  <li><b>子テスト集合作成</b>ボタンをクリックし、テスト集合を作成します。" .
  "このテスト集合に従ってテストドキュメントが構成されます" .
  "(例えば、機能/非機能などの種別、コンポーネントやフィーチャーによる分別など)。" .
  "テスト集合には、テストケース、環境設定、関連ドキュメントへの参照、" .
  "制限事項や、それ以外に必要な情報を記載します。" .
  "一般に、配下のテストケースに関する様々な共通する説明を記述することになります。" .
  "<li>テスト集合は「フォルダー」のような概念ともいえます。 従って、プロジェクト内で" .
  "テスト集合のコピーや移動ができます。また、インポートとエクスポートも可能です (テストケースを含めることもできます)。</li>
  <li>作成したテスト集合をナビゲーションから選択し、" .
  "<b>テストケースを作成</b>ボタンをクリックすることでテストケースを作成します。" .
  "テストケースには、テストシナリオ、期待結果やプロジェクトで定義されたカスタムフィールド" .
...
<p>TestLinkではテスト集合によってテストケースを整理することができるでしょう。" .
"テスト集合は入れ子状にすることができるので、テスト集合の階層を作ることができます。
...
検索結果は、現在のプロジェクトのテストケースのみを含みます。</p>
...
<p>この機能は各々のテストケース、またはプロジェクト/テスト計画全体のテストケースを印刷できます。</p>
<h2>やってみましょう！:</h2>
<ol>
<li>
<p>参照するテストケース、テスト集合、またはプロジェクトをクリックします。<br />
</p>
</li>
...
<p>各要件は、プロジェクトごとに作成する<b>要件仕様ドキュメント</b>によって
...
<p>要件を作成する要件仕様のタイトルをクリックしてください。要件仕様が内場合は、プロジェクト名をクリックすることで作成することができます。
...
これらのテストケースは、TestLinkの設定で定義された名前のテスト集合に格納されます
...
またはプロジェクトのすべての要件を印刷できます。</p>
<h2>最初に:</h2>
<ol>
<li>
<p>表示する要件の部分を選択し、要件、要件仕様、またはプロジェクトをクリックします。
印刷可能なページが表示されます。</p>
</li>
...
<p>キーワードの割り当てでは、ユーザが一括してテスト集合またはテストケースに
...
<h3>一般的なテスト計画指標</h3>
<p>このページでは、テスト集合、オーナー、キーワードごとのテスト計画の最新の
...
<p>以下の表が表示されます:</p>
<ul>
    <li><b>最上位のテスト集合ごとの結果</b>
    各最上位集合の結果が一覧表示されます。成功、失敗、ブロック、未実行のテストケースの合計、そして完了率を表示します。
    「完了した」テストケースとは、成功、失敗、ブロックの結果となったテストケースのことです。
    トップレベル集合の結果には全ての子集合の結果を含みます。</li>
    <li><b>キーワードごとの結果</b>
...
コントロールを変更することにより、指定したオーナー、キーワード、テスト集合、
...
<li><b>トップレベルテスト集合:</b> 0個からn個のトップレベルテスト集合を選択することができます。デフォルトでは全てのテスト集合が選択されています。選択したテスト集合の結果のみが、クエリーの指標に反映されます。特定のテスト集合の結果に興味がある場合は、このコントロールを操作してください。</li>
...
キーワード、オーナー、トップレベルテスト集合を選択することで、テスト計画ごと、テスト集合ごとの指標計算に使用するテストケースの数を変化させることができます。例えば、オーナー=「Greg」、キーワード=「優先度1」と全テスト集合を選択した場合は、Gregに割り当てられた優先度1のテストケースのみが計算に使用されます。レポートの「テストケース数」の合計は、これら3種類のコントロールによって決められます。
...
<li>テスト計画内の合計</li>
<li>集合ごとの全実行結果の合計 (総テストケース数 / 成功 / 失敗 / ブロック / 未実行)。もし、あるテストケースを複数のビルドで複数回実行した場合、選択したビルドに関連する全ての実行結果を表示します。しかし、テスト集合の実行結果の概要には、選択したビルドに対する「最新のテスト結果」のみが反映されます。</li>
</ol>
...
<h3>各テストケースのバグの合計</h3>
<p>このレポートは、プロジェクト内のバグが発見されたテストケースを表示します。このレポートはバグトラキングシステムが接続されている場合のみ表示されます。</p>";
...
<h2>テストケースの追加/削除:</h2>
<ol>
  <li>テスト集合をクリックし、全て、または1つのテストケースを表示します。</li>
...
<p>TestLinkでは、テストケースの優先度に影響を与える「緊急度」をテスト集合に設定することができます。
...
<p><i>一例として、テストケースに重要度「高」、テスト集合に緊急度「低」を設定すると " .
    "優先度は「中」になります。</i>";
