...
<h3>テスト実行</h3>
<p>このページでは、ユーザがテストケースを実行し（テスト結果を登録し）、テスト計画のテストケース集合を印刷できます。
...
リーダー権限を持つユーザは、優先度、リスク、テストケース集合（カテゴリー）の所有権と、
...
<li>カスタムフィールドはひとつのテスト要素に関連付けられます (テスト集合, テストケース)</li>
<li>カスタムフィールドは複数のプロジェクトの要素に関連付ける事ができます。</li>
<li>カスタムフィールドの表示順はプロジェクトごとに異なります。</li>
<li>特定のプロジェクトに対して、あるカスタムフィールドを無効にすることができます。</li>
...
カスタムの関連と、異なるプロジェクトの要件間の関連を許可するオプションは、
設定ファイルで設定できます。
...
例えば、以下のカスタムフィールドをプロジェクトで定義およびアサインするとします。<br />
